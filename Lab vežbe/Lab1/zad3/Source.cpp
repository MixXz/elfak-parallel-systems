...
void main(int argc, char** argv) {
	...
	if (rank == MASTER) {
		...
		MPI_Type_vector(N - rank, 1, N, MPI_INT, &recvType);
		MPI_Type_commit(&recvType);

		MPI_Recv(&mat[0][0], 1, recvType, MASTER, RIGHT_TAG, MPI_COMM_WORLD, &status);
		MPI_Recv(&mat[0][1], 1, recvType, MASTER, LEFT_TAG, MPI_COMM_WORLD, &status);
		printMatrix(mat, rank);
	}
